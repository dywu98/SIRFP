...
SNAPSHOT_DIR=ckpt/${DATASET}/segformer_noprune
MASTER_ADDR=${MASTER_ADDR:-"127.0.0.1"}
PORT=${PORT:-39888}
PRUNE_METHOD='graph_pruning'
...
ALIGN_CORNER='True'
PRUNE_RATIO=0.1
SNAPSHOT_DIR=ckpt/CS/segformer_noprune_segformer/simi_prune_06
RESUME_DIR=ckpt/CS/segformer_noprune_segformer/CS_scenes_4000.pth
SCORE_DIR=ckpt/CS/segformer_noprune_segformer/score.pth
...
SNAPSHOT_DIR=ckpt/${DATASET}/segformer_noprune
RESUME=ckpt/CS/segformer_noprune_segformer/CS_scenes_4000.pth
# CHANNEL_CFG=ckpt/CS/segformer_noprune_segformer/channel_cfg.pth
MASTER_ADDR=${MASTER_ADDR:-"127.0.0.1"}
PORT=${PORT:-29888}

# python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} --master_addr=$MASTER_ADDR --master_port=32146 train_simi_flex.py --dataset ${DATASET} --balance ${BALANCE} --data-para ${DATA_PARA} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --loss-type ${LOSS_TYPE} --loss-para ${LOSS_PARA} --random-mirror --random-brightness --random-scale --optim ${OPTIM} --learning-rate ${LEARNING_RATE} --warmup ${WARMUP} --weight-decay ${WEIGHT_DECAY} --num-workers ${NUM_WORKERS} --num-steps ${NUM_STEPS} --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --batch-size ${BS} --random-seed 42 --snapshot-dir ${SNAPSHOT_DIR}_${MODEL_NAME} --save-pred-every ${SAVE_PRED_EVERY} --save-steps ${SAVE_STEPS} --resume ${RESUME} --prune-type ${PRUNE_TYPE} --prune-method ${PRUNE_METHOD} 

#ss test
# python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} --master_port=32146 evaluate_withflops_acc.py --dataset ${DATASET} --model ${MODEL_NAME} --backbone ${BACKBONE} --batch-size ${TEST_BS} --whole 'True' --flip 'False' --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --ms '1' --num-workers ${NUM_WORKERS} --restore-from ${SNAPSHOT_DIR}_${MODEL_NAME}/${DATASET}_scenes_${NUM_STEPS}.pth --save-predict 'False' --channel-cfg ${CHANNEL_CFG}
