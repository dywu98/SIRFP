...
ALIGN_CORNER='True'
PRUNE_RATIO=0.8
SNAPSHOT_DIR=ckpt/CS/graph_edge_pretrain_deeplabv3/simi_prune_08
...
RESUME=ckpt/CS/graph_edge_pretrain_deeplabv3/simi_prune_08/pruned.pth
CHANNEL_CFG=ckpt/CS/graph_edge_pretrain_deeplabv3/simi_prune_08/channel_cfg.pth
MASTER_ADDR=${MASTER_ADDR:-"127.0.0.1"}
PORT=${PORT:-29888}

python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} --master_addr=$MASTER_ADDR --master_port=$PORT train_simi_flex.py --dataset ${DATASET} --balance ${BALANCE} --data-para ${DATA_PARA} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --loss-type ${LOSS_TYPE} --loss-para ${LOSS_PARA} --random-mirror --random-brightness --random-scale --optim ${OPTIM} --learning-rate ${LEARNING_RATE} --warmup ${WARMUP} --weight-decay ${WEIGHT_DECAY} --num-workers ${NUM_WORKERS} --num-steps ${NUM_STEPS} --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --batch-size ${BS} --random-seed 42 --snapshot-dir ${SNAPSHOT_DIR}_${MODEL_NAME}_08 --save-pred-every ${SAVE_PRED_EVERY} --save-steps ${SAVE_STEPS} --resume ${RESUME} --channel-cfg ${CHANNEL_CFG}
