...
SNAPSHOT_DIR=ckpt/${DATASET}/simi2_pretrain_0612
...
ALIGN_CORNER='True'
PRUNE_RATIO=0.6
SNAPSHOT_DIR=ckpt/CS/simi2_pretrain_0612_deeplabv3/simi_prune_06
RESUME_DIR=ckpt/CS/simi2_pretrain_0612_deeplabv3/CS_scenes_4000.pth
SCORE_DIR=ckpt/CS/simi2_pretrain_0612_deeplabv3/score.pth

# python prune_simi2.py --dataset ${DATASET} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --align-corner ${ALIGN_CORNER} --prune-ratio ${PRUNE_RATIO} --save-path ${SNAPSHOT_DIR} --model-path ${RESUME_DIR} --score-path ${SCORE_DIR} 
...
SNAPSHOT_DIR=ckpt/${DATASET}/simi2_finetune_0612
RESUME=ckpt/CS/simi2_pretrain_0612_deeplabv3/simi_prune_06/pruned.pth
CHANNEL_CFG=ckpt/CS/simi2_pretrain_0612_deeplabv3/simi_prune_06/channel_cfg.pth
MASTER_ADDR=${MASTER_ADDR:-"127.0.0.1"}
PORT=${PORT:-29888}
...
python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} --master_port=12345 evaluate_onlyflops.py --dataset ${DATASET} --model ${MODEL_NAME} --backbone ${BACKBONE} --batch-size ${TEST_BS} --whole 'True' --flip 'False' --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --ms '1' --num-workers ${NUM_WORKERS} --restore-from ${SNAPSHOT_DIR}_${MODEL_NAME}/${DATASET}_scenes_${NUM_STEPS}.pth --save-predict 'False' --channel-cfg ${CHANNEL_CFG}
