...
SNAPSHOT_DIR=ckpt/${DATASET}/simi8canny0.5_pretrain

# #train
# python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} train_simi8.py --dataset ${DATASET} --balance ${BALANCE} --data-para ${DATA_PARA} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --loss-type ${LOSS_TYPE} --loss-para ${LOSS_PARA} --random-mirror --random-brightness --random-scale --shortsize ${SHORT_SIZE} --longsize ${LONG_SIZE} --optim ${OPTIM} --learning-rate ${LEARNING_RATE} --warmup ${WARMUP} --weight-decay ${WEIGHT_DECAY} --num-workers ${NUM_WORKERS} --num-steps ${NUM_STEPS} --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --batch-size ${BS} --random-seed 42 --snapshot-dir ${SNAPSHOT_DIR}_${MODEL_NAME} --save-pred-every ${SAVE_PRED_EVERY} --save-steps ${SAVE_STEPS} --prune-type ${PRUNE_TYPE}
...
ALIGN_CORNER='False'
PRUNE_RATIO=0.6
SNAPSHOT_DIR=ckpt/ADE/simi8canny0.5_pretrain_deeplabv3/simi8canny0.5_prunesimi1_06
RESUME_DIR=ckpt/ADE/simi8canny0.5_pretrain_deeplabv3/ADE_scenes_16000.pth
SCORE_DIR=ckpt/ADE/simi8canny0.5_pretrain_deeplabv3/score.pth

# python prune_simi1.py --dataset ${DATASET} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --align-corner ${ALIGN_CORNER} --prune-ratio ${PRUNE_RATIO} --save-path ${SNAPSHOT_DIR} --model-path ${RESUME_DIR} --score-path ${SCORE_DIR} 
...
# SAVE_PRED_EVERY=5000
# SAVE_STEPS=$(($NUM_STEPS - `expr 7 \* ${SAVE_PRED_EVERY}`))
# SNAPSHOT_DIR=ckpt/${DATASET}/simi8canny0.5_finetune
# RESUME=ckpt/ADE/simi8canny0.5_pretrain_deeplabv3/simi8canny0.5_prune_06/pruned.pth
# CHANNEL_CFG=ckpt/ADE/simi8canny0.5_pretrain_deeplabv3/simi8canny0.5_prune_06/channel_cfg.pth
