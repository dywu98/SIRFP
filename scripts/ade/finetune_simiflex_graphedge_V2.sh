...
SNAPSHOT_DIR=ckpt/${DATASET}/graph_edge_V2.1_pretrain
...
ALIGN_CORNER='False'
PRUNE_RATIO=0.6
SNAPSHOT_DIR=ckpt/ADE/graph_edge_V2.1_pretrain_deeplabv3/graph_edge_V2.1_prune_06
RESUME_DIR=ckpt/ADE/graph_edge_V2.1_pretrain_deeplabv3/ADE_scenes_16000.pth
SCORE_DIR=ckpt/ADE/graph_edge_V2.1_pretrain_deeplabv3/score.pth
...
SAVE_PRED_EVERY=5000
SAVE_STEPS=$(($NUM_STEPS - `expr 7 \* ${SAVE_PRED_EVERY}`))
SNAPSHOT_DIR=ckpt/${DATASET}/graph_edge_V2_finetune
RESUME=ckpt/ADE/graph_edge_V2_pretrain_deeplabv3/graph_edge_V2_prune_06/pruned.pth
CHANNEL_CFG=ckpt/ADE/graph_edge_V2_pretrain_deeplabv3/graph_edge_V2_prune_06/channel_cfg.pth
...
python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} evaluate_visgt.py --dataset ${DATASET} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --batch-size ${TEST_BS} --shortsize ${SHORT_SIZE} --longsize ${LONG_SIZE} --whole 'True' --flip 'False' --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --ms '1' --num-workers ${NUM_WORKERS} --restore-from ${SNAPSHOT_DIR}_${MODEL_NAME}/${DATASET}_scenes_${NUM_STEPS}.pth --save-predict 'True' --channel-cfg ${CHANNEL_CFG}


#ms test
WHOLE='True'; MS='0.5,0.75,1,1.25,1.5,1.75'; FLIP='True'
# python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} evaluate_amp.py --dataset ${DATASET} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --batch-size ${TEST_BS} --shortsize ${SHORT_SIZE} --longsize ${LONG_SIZE} --whole ${WHOLE} --flip ${FLIP} --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --ms ${MS} --num-workers ${NUM_WORKERS} --restore-from ${SNAPSHOT_DIR}_${MODEL_NAME}/${DATASET}_scenes_${NUM_STEPS}.pth --save-predict 'True' --channel-cfg ${CHANNEL_CFG}
# python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} evaluate_amp.py --dataset ${DATASET} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --batch-size ${TEST_BS} --shortsize ${SHORT_SIZE} --longsize ${LONG_SIZE} --whole ${WHOLE} --flip ${FLIP} --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --ms ${MS} --num-workers ${NUM_WORKERS} --restore-from ckpt/ADE/graph_edge_V2_finetune_deeplabv3/ADE_scenes_144000.pth --save-predict 'True' --channel-cfg ckpt/ADE/graph_edge_V2_finetune_deeplabv3/channel_cfg.pth
