...
python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} --master_port=32146 evaluate_withflops.py --dataset ${DATASET} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --batch-size ${TEST_BS} --shortsize ${SHORT_SIZE} --longsize ${LONG_SIZE} --whole 'True' --flip 'False' --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --ms '1' --num-workers ${NUM_WORKERS} --restore-from ckpt/ADE/graph_edge_multi_finetune_iter2_deeplabv3/ADE_scenes_140000.pth --save-predict 'False' --channel-cfg ckpt/ADE/graph_edge_multi_finetune_iter2_deeplabv3/channel_cfg.pth
