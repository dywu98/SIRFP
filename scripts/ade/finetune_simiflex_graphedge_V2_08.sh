...
SNAPSHOT_DIR=ckpt/${DATASET}/graph_edge_V2_p07_pretrain
...
ALIGN_CORNER='False'
PRUNE_RATIO=0.8
SNAPSHOT_DIR=ckpt/ADE/graph_edge_V2_p07_pretrain_deeplabv3/graph_edge_V2_p08_prune_08
RESUME_DIR=ckpt/ADE/graph_edge_V2_p07_pretrain_deeplabv3/ADE_scenes_16000.pth
SCORE_DIR=ckpt/ADE/graph_edge_V2_p07_pretrain_deeplabv3/score.pth
...
SAVE_PRED_EVERY=5000
SAVE_STEPS=$(($NUM_STEPS - `expr 7 \* ${SAVE_PRED_EVERY}`))
SNAPSHOT_DIR=ckpt/${DATASET}/graph_edge_V2_p08_finetune
RESUME=ckpt/ADE/graph_edge_V2_p07_pretrain_deeplabv3/graph_edge_V2_p08_prune_08/pruned.pth
CHANNEL_CFG=ckpt/ADE/graph_edge_V2_p07_pretrain_deeplabv3/graph_edge_V2_p08_prune_08/channel_cfg.pth
...
python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} evaluate_withflops.py --dataset ${DATASET} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --batch-size ${TEST_BS} --shortsize ${SHORT_SIZE} --longsize ${LONG_SIZE} --whole 'True' --flip 'False' --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --ms '1' --num-workers ${NUM_WORKERS} --restore-from ckpt/ADE/graph_edge_V2_p08_finetune_deeplabv3_old/ADE_scenes_144000.pth --save-predict 'False' --channel-cfg ${CHANNEL_CFG}
