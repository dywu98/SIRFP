...
PORT=${PORT:-12345}
PRUNE_METHOD='graph_edgev2_pruning'
PRUNE_TYPE='simi'
...
ALIGN_CORNER='True'
PRUNE_RATIO=0.3
SNAPSHOT_DIR=ckpt/COCO/graph_pretrain_deeplabv3/graph_prune_06
...
SAVE_PRED_EVERY=1000
SAVE_STEPS=$(($NUM_STEPS - `expr 7 \* ${SAVE_PRED_EVERY}`))
SNAPSHOT_DIR=ckpt/${DATASET}/graph_edgev2_finetune_iter1
RESUME=ckpt/COCO/graph_pretrain_deeplabv3/graph_prune_06/pruned.pth
CHANNEL_CFG=ckpt/COCO/graph_pretrain_deeplabv3/graph_prune_06/channel_cfg.pth
...
ALIGN_CORNER='True'
PRUNE_RATIO=0.58
SNAPSHOT_DIR=ckpt/COCO/graph_edgev2_finetune_iter1_deeplabv3/graph_prune_06
RESUME_DIR=ckpt/COCO/graph_edgev2_finetune_iter1_deeplabv3/COCO_scenes_6000.pth
SCORE_DIR=ckpt/COCO/graph_edgev2_finetune_iter1_deeplabv3/score.pth
...
SAVE_PRED_EVERY=1000
SAVE_STEPS=$(($NUM_STEPS - `expr 7 \* ${SAVE_PRED_EVERY}`))
SNAPSHOT_DIR=ckpt/${DATASET}/graph_edgev2_finetune_iter2
RESUME=ckpt/COCO/graph_edgev2_finetune_iter1_deeplabv3/graph_prune_06/pruned.pth
CHANNEL_CFG=ckpt/COCO/graph_edgev2_finetune_iter1_deeplabv3/graph_prune_06/channel_cfg.pth


#train
# python -m torch.distributed.launch --nproc_per_node=${GPU_NUM} train.py --dataset ${DATASET} --balance ${BALANCE} --data-para ${DATA_PARA} --model ${MODEL_NAME} --model-para ${MODEL_PARA} --backbone ${BACKBONE} --backbone-para ${BACKBONE_PARA} --loss-type ${LOSS_TYPE} --loss-para ${LOSS_PARA} --random-mirror --random-brightness --random-scale --shortsize ${SHORT_SIZE} --longsize ${LONG_SIZE} --optim ${OPTIM} --learning-rate ${LEARNING_RATE} --warmup ${WARMUP} --weight-decay ${WEIGHT_DECAY} --num-workers ${NUM_WORKERS} --num-steps ${NUM_STEPS} --input-size ${INPUT_SIZE} --align-corner ${ALIGN_CORNER} --batch-size ${BS} --random-seed 42 --snapshot-dir ${SNAPSHOT_DIR}_${MODEL_NAME} --save-pred-every ${SAVE_PRED_EVERY} --save-steps ${SAVE_STEPS} --resume ${RESUME} --channel-cfg ${CHANNEL_CFG} --prune-type ${PRUNE_TYPE}
...
ALIGN_CORNER='True'
PRUNE_RATIO=0.60
SNAPSHOT_DIR=ckpt/COCO/graph_edgev2_finetune_iter1_deeplabv3/graph_prune_06_real
RESUME_DIR=ckpt/COCO/graph_edgev2_finetune_iter1_deeplabv3/COCO_scenes_6000.pth
SCORE_DIR=ckpt/COCO/graph_edgev2_finetune_iter1_deeplabv3/score.pth
...
SAVE_PRED_EVERY=1000
SAVE_STEPS=$(($NUM_STEPS - `expr 7 \* ${SAVE_PRED_EVERY}`))
SNAPSHOT_DIR=ckpt/${DATASET}/graph_edgev2_finetune_iter2_real06
RESUME=ckpt/COCO/graph_edgev2_finetune_iter1_deeplabv3/graph_prune_06_real/pruned.pth
CHANNEL_CFG=ckpt/COCO/graph_edgev2_finetune_iter1_deeplabv3/graph_prune_06_real/channel_cfg.pth
