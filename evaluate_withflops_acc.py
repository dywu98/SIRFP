...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        if args.use_trt:
            from torch2trt_git import TRTModule
            seg_model = TRTModule()
        ...
        for idx in pbar:
            ...
            labeled += np.sum(ignore_index)
            correct += np.sum(seg_gt==seg_pred)
        ...
        if (not engine.distributed) or (engine.distributed and engine.local_rank == 0):
            ...
            with open(os.path.join(model_path, 'result.txt'), 'a') as f:
                ...
                f.write(json.dumps({'PixAcc':correct/labeled}))
                f.write('\n')
                f.write('params: {}, params2: {}, \n'.format(params, params2))
                f.write('flops: {}, flops2: {}, \n'.format(flops, flops2))
                f.write('--------\n')
        print('PixAcc:', correct/labeled)
